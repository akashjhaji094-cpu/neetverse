...
export const OfflinePaperPreview = ({
 ...
 const queryClient = useQueryClient();
    ...
    const load = async () => {
      ...
      if (!MJ) {
        ...
        await new Promise<void>((resolve) => {
          ...
          const script = document.createElement("script");
          // CHTML output — this is what's used for the on-screen paper AND
          // for Print (native browser print, which renders this exactly as
          // shown). We tried switching to SVG output earlier to help canvas
          // rendering for "Download PDF" — but SVG math glyphs render as
          // solid filled paths with no font hinting, which made formulas
          // look noticeably darker/bolder than the surrounding serif text
          // when printed. CHTML matches the surrounding text weight
          // correctly. "Download PDF" no longer uses canvas rasterization
          // at all (see handleDownloadPdf below), so there's no longer any
          // reason to compromise print quality for it.
  ...
  /**
   * "Download PDF" — after three rounds of html2canvas-based rasterization
   * (missing images, garbled math, split questions, and finally a fully
   * blank PDF), that approach has proven too fragile for a document this
   * complex. Browsers deliberately don't let JS pick a print destination
   * (security), so there is no way to silently "just download a PDF"
   * without either (a) a canvas snapshot — fragile, as we've now seen
   * repeatedly — or (b) a real backend PDF renderer (out of scope here).
   *
   * So this now reuses the exact same native browser print pipeline as
   * the Print button — which already renders this page's real fonts, real
   * MathJax CHTML math, and real images with 100% correct pagination,
   * because it's the browser's own layout engine doing it, not a
   * screenshot. We simply open the print dialog and tell the user to
   * choose "Save as PDF" as the destination. It's one extra tap, but it
   * guarantees the output actually matches what's on screen — every time.
   */
  const handleDownloadPdf = useCallback(() => {
    toast({
      title: 'In the dialog, choose "Save as PDF"',
      description: 'Set Destination / Printer to "Save as PDF" and tap Save to download the file.',
    });
    window.print();
  }, [toast]);
      ...
      <div className="print:hidden sticky top-0 z-50 bg-white border-b shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" onClick={handlePrint} disabled={!ready} size="sm" className="gap-2">
              <Printer className="h-4 w-4" />
              <span className="hidden sm:inline">Print</span>
            </Button>
            <Button onClick={handleDownloadPdf} disabled={!ready} variant="outline" size="sm" className="gap-2">
            ...
            <Button onClick={() => setShowTimerPrompt(true)} disabled={!ready} size="sm" className="gap-2">
