...
export const OfflinePaperPreview = ({
 ...
 const queryClient = useQueryClient();
  ...
  // PRINT
  // ------------------------------------------------------------------
  // FIX: window.print() was immediately followed by setShowTimerPrompt(true).
  // That React re-render could land WHILE the browser was still capturing
  // the page for the print dialog, so what actually got printed was the
  // "Ready to Begin?" timer screen instead of the paper. We now flip to
  // the timer prompt only AFTER the print dialog has genuinely closed
  // (the `afterprint` event), with a timeout fallback for print-to-PDF
  ...
  const handlePrint = useCallback(() => {
    ...
    setTimeout(showPrompt, 1500); // fallback for browsers that skip afterprint
    window.print();
  }, []);
