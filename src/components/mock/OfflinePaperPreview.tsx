...
export const OfflinePaperPreview = ({
 ...
 const [ready, setReady] = useState(false);
  ...
  // NEW: while true, the paper temporarily renders in single-column layout
  // so html2canvas (which doesn't reliably support CSS multi-column) can
  // capture it cleanly for the downloaded PDF. Never affects Print (which
  // uses the browser's own native @media print engine, not html2canvas).
 ...
 const queryClient = useQueryClient();
  ...
  // PRINT
  // ------------------------------------------------------------------
  // FIX: previously `setShowTimerPrompt(true)` was called immediately after
  // `window.print()`. Because window.print() only BLOCKS the page while the
  // native print dialog is open on some browsers (and doesn't block at all
  // on others), the React re-render triggered by setShowTimerPrompt could
  // land while the browser was still capturing the page for print/PDF —
  // so what actually got printed was the "Ready to Begin?" timer prompt
  // screen instead of the actual question paper. Now we only flip to the
  // timer prompt AFTER the print dialog has genuinely closed, using the
  // `afterprint` event, with a timeout fallback for browsers/print-to-PDF
  // flows that don't fire it reliably.
  // ------------------------------------------------------------------
  const handlePrint = useCallback(() => {
    ...
    window.addEventListener("afterprint", showPrompt, { once: true });
    // Fallback: some browsers / "Save as PDF" print flows never fire
    // afterprint reliably — don't leave the user stuck with no prompt.
   ...
   *
   * FIX (this version):
   *  1. Images are force-set to crossOrigin="anonymous" and re-awaited for
   *     full decode before capture — previously html2canvas would silently
   *     skip/blank images that weren't explicitly marked CORS-clean, which
   *     is why question images were missing from downloaded PDFs.
   *  2. The questions section briefly switches from CSS multi-column to
   *     single-column during capture (`capturing` state), since html2canvas
   *     does not reliably support CSS `column-count` layouts — this is what
   *     caused the "ajeeb sa" garbled/overlapping downloaded PDF.
   *  3. We wait on `document.fonts.ready` + a short delay after MathJax
   *     typesetting so LaTeX/MathJax glyphs are fully painted (as real
   *     pixels) before the canvas snapshot is taken, fixing broken/garbled
   *     math in the downloaded PDF.
   */
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      // 1) Force single-column layout for a clean, linear html2canvas capture
      setCapturing(true);
      await new Promise((r) => setTimeout(r, 80));

      // 2) Make every image CORS-clean + fully decoded before capture
      ...
      await Promise.all(
        imgs.map((img) => {
          ...
          return new Promise<void>((resolve) => {
            ...
            img.onload = done;
            img.onerror = done;
            // re-trigger load now that crossOrigin is set, in case the
            // browser already cached a non-CORS version of this image
            const src = img.src;
            img.src = "";
            img.src = src;
            // safety timeout so one stuck image can't hang the whole export
            setTimeout(done, 8000);
          });
        })
      );

      // 3) Let web fonts / MathJax glyphs finish painting as real pixels
          ...
          {subjectGroups.map((group) => {
            ...
            return (
              <div key={group.name} className="mb-3">
                ...
                <div
                  style={{
                    // NEW: single-column while the PDF snapshot is being
                    // captured (html2canvas doesn't reliably support CSS
                    // multi-column layouts) — reverts automatically once
                    // the capture finishes. Screen view + Print are
                    // unaffected (Print uses the browser's native print
                    // engine, not this state).
                    columnCount: capturing ? 1 : 2,
                    columnGap: "8mm",
