...
function TestsPanel({ seriesId, onBack, onOpenQuestions }: {
  ...
  // NEW: which restricted test's "Manage students" dialog is open (post-creation)
  ...
  const createTest = async () => {
    ...
    // REMOVED: previously required `selectedAssignees.length` to be > 0
    // for a restricted test. Admin can now create a restricted test with
    // ZERO students assigned and add them later via "Manage students"
    // (below) any time before the test's start time.
    ...
    toast.success(
      isRestricted && !selectedAssignees.length
        ? "Test ban gaya — ab jab chaho 'Manage students' se users add kar sakte ho"
        : "Test add ho gaya"
    );
  };
  ...
  return (
    <div className="space-y-5">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <Label className="flex items-center gap-1.5">
                      <Users className="h-3.5 w-3.5" /> Select users ({selectedAssignees.length} selected) — optional, baad me bhi add kar sakte ho
                    </Label>
              ...
              : tests.map((t: any) => (
                <div key={t.id} className="rounded-xl border p-4 space-y-2">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {/* NEW: manage assignees any time before start_at (or view/remove after) */}
                    {t.is_restricted && (
                      <Button size="sm" variant="outline" className="gap-1.5" onClick={() => setManageTestId(t.id)}>
      ...
      <ManageAssigneesDialog
        testId={manageTestId}
        tests={tests || []}
        onClose={() => setManageTestId(null)}
      />
    </div>
  );
}

/* ================= manage assignees on an existing restricted test ================= */
function ManageAssigneesDialog({ testId, tests, onClose }: {
  testId: string | null; tests: any[]; onClose: () => void;
}) {
  const qc = useQueryClient();
  ...
  const started = test?.start_at ? new Date() > new Date(test.start_at) : false;
  ...
  const { data: current, isLoading } = useQuery({
    queryKey: ["series-test-assignees", testId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("series_test_assignees").select("user_id").eq("test_id", testId as string);
      if (error) throw error;
  ...
  const save = async () => {
    if (!testId || !current) return;
    setBusy(true);
    const toAdd = checked.filter((id) => !current.includes(id));
    const toRemove = current.filter((id) => !checked.includes(id));
    try {
      if (toAdd.length) {
        const { error } = await supabase.from("series_test_assignees")
          .insert(toAdd.map((user_id) => ({ test_id: testId, user_id })));
        if (error) throw error;
      }
      if (toRemove.length) {
        const { error } = await supabase.from("series_test_assignees")
          .delete().eq("test_id", testId).in("user_id", toRemove);
        if (error) throw error;
      }
      toast.success("Students update ho gaye");
      qc.invalidateQueries({ queryKey: ["series-test-assignees", testId] });
      onClose();
    } catch (e: any) {
      console.error(e);
  ...
  return (
    <Dialog open={!!testId} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Manage students — {test?.title}</DialogTitle>
        </DialogHeader>

        {started && (
          <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-2.5">
            Ye test already start ho chuka hai — naye students add nahi honge, lekin existing list dekh aur remove kar sakte ho.
          </p>
        )}
        ...
        {isLoading ? (
          ...
          <div className="max-h-72 overflow-y-auto rounded-xl border divide-y">
            {filtered.map((u: any) => {
              ...
              const disableAdd = started && !isChecked;
              return (
                <label key={u.id} className={`flex items-center gap-2 p-2.5 text-sm ${disableAdd ? "opacity-50" : ""}`}>
                  <Checkbox
                    checked={isChecked}
                    disabled={disableAdd}
                    onCheckedChange={(v) =>
                      setChecked(v ? [...checked, u.id] : checked.filter((x) => x !== u.id))
                    }
                  />
                  <span className="truncate">{u.name || "—"} <span className="text-muted-foreground">({u.email})</span></span>
                </label>
            ...
            {!filtered.length && <p className="p-3 text-sm text-muted-foreground">Koi user nahi mila</p>}
          </div>
        )}

        <Button onClick={save} disabled={busy || isLoading} className="gap-2">
          {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Users className="h-4 w-4" />} Save
        </Button>
      </DialogContent>
    </Dialog>
  );
