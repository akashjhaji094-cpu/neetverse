 ...
 */
export const PracticeLoading = ({ totalQuestions, chapterName }: PracticeLoadingProps) => {
  const [fetched, setFetched] = useState(0);
  const [elapsed, setElapsed] = useState(0);

  // Estimated total time: ~80ms per question + 1.5s overhead, capped 3-15s
  const estimated = Math.min(15, Math.max(3, Math.round((totalQuestions * 0.08) + 1.5)));
  ...
  }, [fetched, totalQuestions]);

  const eta = Math.max(0, estimated - elapsed);
  const pct = Math.round((fetched / totalQuestions) * 100);
        ...
        <CardContent className="pt-8 pb-8 space-y-6">
          <div className="flex flex-col items-center gap-3">
            ...
            <div className="text-center">
              <h3 className="text-xl font-bold">Preparing Your Practice</h3>
              {chapterName && (
          ...
          <div className="text-center space-y-1">
            <div className="text-4xl font-black tabular-nums text-primary">
              {fetched} <span className="text-muted-foreground/60 text-2xl font-bold">/ {totalQuestions}</span>
            </div>
