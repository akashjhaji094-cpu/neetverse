...
out = out.replace(/^{([^}]+)}/g, (, g) => [...g].map((c: string) => SUP[c] ?? c).join(""));
out = out.replace(/^(\S)/g, (, c) => SUP[c] ?? c);
out = out.replace(/{([^}]+)}/g, (, g) => [...g].map((c: string) => SUB[c] ?? c).join(""));
out = out.replace(/(\S)/g, (, c) => SUB[c] ?? c);
